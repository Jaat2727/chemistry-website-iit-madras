...
import { Search, FlaskConical, GraduationCap, Atom, Phone, Mail } from 'lucide-react';
...
const CATEGORIES = ['All', 'Administrative', 'Technical', 'Support'];
            ...
            <div className="container relative z-20 mx-auto px-6 pt-24 pb-20 max-w-7xl">
                ...
                <header className="mb-12">
                    ...
                    <motion.h1 variants={mechanicalReveal} initial="hidden" animate="visible" className="text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[0.9] text-[#1f2937] mb-6 uppercase">
                        STAFF <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-[#b45309]">DIRECTORY</span>
                    </motion.h1>
                    ...
                    <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 1.2 }} className="text-lg md:text-xl font-medium text-[#4b5563] max-w-2xl leading-relaxed">
                        The dedicated administrative and technical staff supporting the operations and research activities of the Chemistry Department.
                    </motion.p>
                </header>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }} className="mb-8">
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {CATEGORIES.map(c => {
                                const bc = CATEGORY_COLORS[c];
                                return (
                                    <button
                                        key={c}
                                        onClick={() => setCategory(c)}
                                        className={`px-5 py-2 text-sm font-semibold rounded-full transition-all duration-300 border ${category === c
                    ...
                    <p className="text-xs text-slate-400 mt-3 font-medium">{filtered.length} staff member{filtered.length !== 1 ? 's' : ''} found</p>
                </motion.div>
                ...
                >
                    {filtered.map((s) => {
                        const bc = CATEGORY_COLORS[s.category] || CATEGORY_COLORS.Administrative;
                        return (
                            <motion.div key={s.id} variants={slowReveal}>
                                <GlowCard glowColor={bc.glow}>
                                    <div
                                        className={`group flex flex-col h-full bg-white/70 backdrop-blur-xl border border-slate-200/80 rounded-3xl p-5 ${bc.hoverBorder} hover:shadow-xl ${bc.hoverShadow} transition-all duration-300 hover:-translate-y-1`}
                                    >
                                        <div className="flex items-center gap-4 mb-4">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <h3 className={`text-[14px] font-bold text-[#1f2937] ${bc.hoverText} transition-colors leading-tight`}>{s.name}</h3>
                                                <p className="text-[12px] text-slate-500 mt-0.5 font-medium">{s.title}</p>
                                                <span className={`inline-block mt-2 text-[10px] font-bold uppercase tracking-wider ${bc.text} ${bc.bg} px-2 py-0.5 rounded-md ${bc.border}`}>{s.category}</span>
                                            </div>
                                        </div>
